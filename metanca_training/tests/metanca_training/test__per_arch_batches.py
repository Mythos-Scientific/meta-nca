...
def test_dict_mode_two_vocabs_smoke():
    ...
    models = [build_tiny_lm(LLMArch(32, 2, 512, 1), 16), build_tiny_lm(LLMArch(32, 2, 1024, 1), 16)]
    test_model = build_tiny_lm(LLMArch(32, 4, 1024, 1), 16)
...
def test_dict_mode_lm_val_factor_smoke():
    """`lm_val_factor` switches in-loop metrics/checkpointing to perplexity/bpb + neg_loss
    (see `_add_lm_metrics`/`checkpoint_monitor` in `_train_metanca.py`) instead of token
    accuracy. This smoke-runs 1 metaepoch of dict-mode training with the flag set, reusing
    the same two-vocab batch structure as `test_dict_mode_two_vocabs_smoke`."""
    register_configs()
    with initialize_config_dir(version_base=None, config_dir=CONFIG_DIR):
        cfg = compose(config_name="config", overrides=[
            "dataset=fashion_mnist", "wandb=disabled",       # dataset fields unused in dict-mode
            "training.num_metaepochs=1", "training.update_step_scheduler_max_steps=1",
            "training.early_stopping_enabled=false", "checkpoint.run_name=pytest_lm_val_factor_smoke",
            "logging.log_every_n_metaepochs=1", "dataset.input_shape=[16]",
        ])
    wandb.init(mode="disabled")
    tb = {512: _fake_lm_batches(512), 1024: _fake_lm_batches(1024, seed=1)}
    ...
    params, tvars = train_metanca(
        tb, vb, cfg=cfg, models=models, test_model=test_model,
        arch_keys=[512, 1024, 1024], lm_val_factor=0.5,
    )
    assert params is not None and hasattr(tvars, "test_tasknet")
